...
``` {r echo=F, include=T}
Keep<-ncol(sampleData[,(colSums(sampleData)> minCoverage)])
Remove<-ncol(sampleData[,(colSums(sampleData)< minCoverage)])
print(paste0("From the total of ", ncol(sampleData), " samples, ", Remove, " samples had to be removed due to low sequencing depth (<5M) -> ", Keep, " samples remaining"))
